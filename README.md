...
> 因为 ngrok（方式 C）已经能满足需求，而且 Cloudflare 临时隧道在**部分校园网 / 公司网**里
> 反复被服务端注销，地址老是变。
>
...
**这些不需要管理员权限。** 如果当前账户是标准用户（非管理员），下面这些做不到：
...
| 关机 | ❌ 换 VPS |
| 合盖待机 | ⚠️ 见上，Modern Standby 机型改不了 |
| 注销 | ✅ 改用 `Win + L` 锁屏 |
...
**⚠️ 内网 DNS 解析不了隧道域名（校园网 / 公司网常见坑）**

内网 / 校园网 DNS 通常是 `10.x` 这类私有地址（比如 `10.0.0.53` / `10.0.0.54`），**解析不了
